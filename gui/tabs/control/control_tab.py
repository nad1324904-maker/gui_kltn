...
class ControlTab(ctk.CTkFrame):
    def __init__(self, parent, main_window=None):
        ...
        # --- Thông số cơ khí (Dùng chung cho các Panel con) ---
        self.l1 = 100.0
        self.l2 = 100.0

        # --- Trạng thái biến Tkinter (Dùng chung) ---
        ...
        if hasattr(self, 'robot_panel') and hasattr(self.robot_panel, 'update_xy_from_joints'):
    ...
    def _create_layout(self):
        ...
        self.status_bar = ctk.CTkLabel(right, text="●  MANUAL MODE", font=FONT_NORMAL, text_color=BTN_PRIMARY)
        self.status_bar.pack(anchor=tk.W, pady=(0, PADDING_SMALL))

        # --- Khung Manual (Mặc định hiển thị) ---
    ...
    def _toggle_mode(self):
        ...
        if self.is_auto_mode:
            ...
            self.mode_btn.configure(text="🤖  AUTO MODE  —  Nhấn để chuyển MANUAL", fg_color=BTN_SUCCESS, hover_color=BTN_SUCCESS_HOVER)
            self.status_bar.configure(text="●  AUTO MODE", text_color=BTN_SUCCESS)
            self.add_log("Chuyển sang AUTO MODE.")
        else:
            # Về chế độ MANUAL
            if hasattr(self.auto_panel, 'auto_state') and getattr(self.auto_panel, 'auto_state') != "IDLE":
                if hasattr(self.auto_panel, '_stop_auto'):
                    self.auto_panel._stop_auto()
            
            self.auto_frame.pack_forget()
            self.manual_frame.pack(fill=tk.X)
            self.mode_btn.configure(text="⚙  MANUAL MODE  —  Nhấn để chuyển AUTO", fg_color=BTN_DEFAULT, hover_color=BTN_DEFAULT_HOVER)
    ...
    def _create_log_section(self, parent):
        ...
        ctk.CTkButton(header, text="CLEAR", width=55, height=22, command=self.clear_log, fg_color=BTN_DEFAULT, hover_color=BTN_DEFAULT_HOVER).pack(side=tk.RIGHT)
    ...
    def notify_stm32_done(self):
        if hasattr(self, 'auto_panel') and hasattr(self.auto_panel, 'notify_stm32_done'):
